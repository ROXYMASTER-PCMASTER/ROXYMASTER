...
async def obtener_tasa_kbt_pen():
    """Devuelve la tasa actual de KBT a PEN usando la formula P_token = (G * beta) / K_total"""
    from variables_globales import G, HH_MULT
    from db import get_db
    db = await get_db()
    # Total KBT circulante = minado - quemado
    total_kbt = await db.fetchval(
        "SELECT COALESCE(SUM(kbt_minado), 0) - COALESCE(SUM(kbt_quemado), 0) FROM perfiles"
    )
    if not total_kbt or total_kbt <= 0:
        return 0.0
    tasa = (G * HH_MULT) / total_kbt
    return round(tasa, 6)


async def obtener_estadisticas_minado():
    """Devuelve estadisticas globales de minado: total minado, tasa actual, etc."""
    from db import get_db
    db = await get_db()
    total_minado = await db.fetchval("SELECT COALESCE(SUM(kbt_minado), 0) FROM perfiles")
    total_quemado = await db.fetchval("SELECT COALESCE(SUM(kbt_quemado), 0) FROM perfiles")
    circulante = total_minado - total_quemado
    tasa = await obtener_tasa_kbt_pen()
    return {
        "total_minado": float(total_minado),
        "total_quemado": float(total_quemado),
        "circulante": float(circulante),
        "tasa_kbt_pen": tasa
    }
