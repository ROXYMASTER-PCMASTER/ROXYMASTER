...
class OrchestratorLocal:
    ...
    def __init__(self, profile_manager: ProfileManager, roxy_api=None, portal_port: int = 8087):
    ...
    async def _cmd_asignar(self, params: dict, cmd_id: str) -> dict:
        """asignar perfiles a una url. formato: 'asignar <cant> url <url> duracion <min>'."""
        ...
        for pid in asignados:
            try:
                success = await self.pm.open_url(pid, url)
                if success:
                    self.pm.set_duracion(pid, duracion)
                    if streamer:
                        self.pm.set_streamer(pid, streamer)
    ...
    async def _cmd_detener(self, params: dict, cmd_id: str) -> dict:
        ...
        for pid in perfiles_ids:
            try:
                await self.pm.redirect_to_portal(
                    pid, f"http://127.0.0.1:{self.portal_port}"
                )
                resultados.append({"perfil": pid, "ok": True})
            except Exception as e:
                resultados.append({"perfil": pid, "ok": False, "error": str(e)})
    ...
    async def _cmd_cambiar_modo(self, params: dict, cmd_id: str) -> dict:
        ...
        ws_ref = params.get("_ws_ref", None)
        if ws_ref:
            ws_ref.cambiar_modo(modo)
